...
<?php
  $lawn_care_content = apply_filters( 'the_content', get_the_content() );
  $lawn_care_video = false;

  // Only get video from the content if a playlist isn't present.
  if ( false === strpos( $lawn_care_content, 'wp-playlist-script' ) ) {
    $lawn_care_video = get_media_embedded_in_content( $lawn_care_content, array( 'video', 'object', 'embed', 'iframe' ) );
  }
?>
  ...
  <div class="post-main-box p-3 mb-3 wow zoomIn" data-wow-duration="2s">
    <?php
      if ( ! is_single() ) {
        ...
        if ( ! empty( $lawn_care_video ) ) {
          foreach ( $lawn_care_video as $lawn_care_video_html ) {
            echo '<div class="entry-video">';
              echo $lawn_care_video_html;
            echo '</div>';
          }
        };
      };
