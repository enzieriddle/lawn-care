...
<?php
  ...
  $lawn_care_audio = false;

  // Only get audio from the content if a playlist isn't present.
  if ( false === strpos( $content, 'wp-playlist-script' ) ) {
    $lawn_care_audio = get_media_embedded_in_content( $content, array( 'audio' ) );
  }
?>
  ...
  <div class="post-main-box p-3 mb-3 wow zoomIn" data-wow-duration="2s">
      <?php
        if ( ! is_single() ) {
          ...
          if ( ! empty( $lawn_care_audio ) ) {
            foreach ( $lawn_care_audio as $lawn_care_audio_html ) {
              echo '<div class="entry-audio">';
                echo $lawn_care_audio_html;
